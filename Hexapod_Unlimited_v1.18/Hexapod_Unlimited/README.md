...
1. **Tujuh panjang ruas masih `-1`: ruas 6, 9, 10, 11, 15, 17, 23.**
   `tabelSiap()` menolak berangkat selama salah satunya kosong. Yang `HNT_ODO`
   (6, 9, 10, 15, 17, 23) diukur dengan `m6 <idx>` lalu ditulis `m7 <idx> <cm>`.
   **Ruas 11 tidak bisa diukur begitu** — ia `HNT_SISI`, yang disetel jarak ke
   dinding SAMPING, bukan jarak tempuh: coba `V<cm>` langsung, lalu tulis
   angkanya. Sebabnya semua kosong: lintasan dari ruas 4 ke bawah dirombak
   6 September (malam) atas arahan operator di arena, dan angka lama diukur
   pada jalur yang sudah tidak ada.
...
8. **Capit belum terpasang fisik.** Ruas korban (`AKS_AMBIL`/`AKS_TARUH`) hanya
   berhenti kosong. Sekuens lengannya sudah ditulis lengkap sebagai komentar di
   `Misi.cpp`. K-4 memang **dilewati dengan sengaja** — dua capit, empat korban,
   dua safe zone; aritmetikanya ada di blok KAPASITAS CAPIT.
