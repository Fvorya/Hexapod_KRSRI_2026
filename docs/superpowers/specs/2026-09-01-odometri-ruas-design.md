...
| Rem tertinggal terpasang | Rem dilepas otomatis saat menyala, dan ikut dilepas oleh `s`, `x`, dan Enter. `m0` SENGAJA tidak melepasnya: `Mission::batal()` sudah memanggil `_nav.navBerhenti()`, dan orang yang membatalkan misi belum tentu ingin rem ukurnya ikut lepas. Sebagai efek samping: rem yang menyala di tengah misi memicu pemeriksaan `navMode() != NAV_ARENA_KANAN` milik `Mission` (`Mission.cpp:242`) dan misi gagal dengan bersih, sebabnya tercetak. |
